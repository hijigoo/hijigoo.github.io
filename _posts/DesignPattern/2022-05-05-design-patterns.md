...
#### 1. Strategy Pattern (#알고리즘 #Family #캡슐화 #교환)
  ...
  - When: 
    1. Runtime에 동적으로 알고리즘을 변경하고자할 때
...
#### 2. Observer Pattern (#Subject #상태변경 #Observer #감지)
  ...
  - When:
    ...
    2. 커뮤니케이션을 위한 Loose Coupling 이 필요할 때
    3. 하나 이상의 객체의 상태 변경이 다른 객체의 동작을 트리거해야할 때
    4. Broadcasting 기능이 필요할 때
...
#### 14. Builder Pattern (#복잡한객체 #동일한절차 #구성요소 #다른표현 #생성방법)
  - Type: Creational
  - Purpose(Benefit)
    1. 여러가지 빌더가 같은 구성 요소 가지고 서로 다른 방법으로 구축할 때 사용하는 패턴
...
#### 15. Singleton Pattern (#시스템 #오직하나)
  ...
  - When:
    ...
    2. 단일 개체에 대한 제어된 액세스가 필요할 때

#### 16. Prototype Pattern (#기존객체 #복사)
  - Type: Creational
  - Purpose(Benefit)
    1. 존재하는 객체를 복사해서 새로운 객체를 만들는 패턴.
...
#### 17. Decorator Pattern (#상속X #Wrapping #Responsiblity #Behavior #추가덧붙임)
  ...
  - When:
    1. subclassing 으로 추가가 거의 불가능할 때
    2. 객체 책임과 행동은 동적으로 수정 가능해야 할 때
    3. 구체적인 구현(implementation)은 책임과 행동에서 분리되어야 할 때
...
#### 21. Facade Pattern (#복잡한Subsystem #하나의Interface)
  ...
  - Purpose(Benefit)
    ...
    3. 서브시스템을 좀 더 사용하기 편하게 만드는 상위 수준의 인터페이스를 정의
  - When:
    1. 너무 많은 인터페이스가 존재할 때 초보자가(?) 쉽게 사용할 수 있도록 만든다
